...
class EM_T_coupler:
    def __init__(
        self,
        transmittersateliteEM,
        transmittersateliteT,
        EMsetparam_key,
        EMgetparam_key,
        EMgetparam_otherparams,
        Tsetparam_key,
        Tgetparam_key,
        Tgetparam_otherparams,
        coupling_interval,
    ):
    ...
    def main(self):
        time = 0
        recindexEM = self.transmittersateliteEM.record(
            self.EMgetparam_key, self.coupling_interval, self.EMgetparam_otherparams
        )
        recindexT = self.transmittersateliteT.record(
            self.Tgetparam_key, self.coupling_interval, self.Tgetparam_otherparams
        )
        ...
        while not (
            self.transmittersateliteEM.finished()
            or self.transmittersateliteT.finished()
        ):  # xxx correct?
            ...
            recindexEM = self.transmittersateliteEM.record(
                self.EMgetparam_key, nexttime, self.EMgetparam_otherparams
            )
            recindexT = self.transmittersateliteT.record(
                self.Tgetparam_key, nexttime, self.Tgetparam_otherparams
            )
            waitindexEM = self.transmittersateliteEM.continue_until(
                nexttime, waitindexEM
            )
