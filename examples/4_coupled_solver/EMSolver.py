...
class EMsolver:
    def __init__(
        self, n, EMinit, dt, EMsource, Tinit, tend, sigma0, sigmaT, transmitter
    ):
        self.EM = EMinit
        self.t = 0
        self.n = n
    ...
    def solveV(self):
        ...
        for j in range(1, self.n - 1):
            ...
            col[counter + 1] = j + 1
            data[counter + 1] = (
                mysigma * self.sigma[0, j + 1] / (mysigma + self.sigma[0, j + 1])
            )
            row[counter + 2] = j
            col[counter + 2] = j + self.n
            data[counter + 2] = (
                mysigma * self.sigma[1, j] / (mysigma + self.sigma[1, j])
            )
            row[counter + 3] = j
            col[counter + 3] = j - 1
            data[counter + 3] = (
            ...
            col[counter] = j
            data[counter] = -(
                2 * mysigma + data[counter + 1] + data[counter + 2] + data[counter + 3]
            )
            counter += 4
        j = self.n - 1
        mysigma = self.sigma[0, j]
        ...
        for i in range(1, self.n - 1):
            ...
            data[counter + 1] = (
                mysigma * self.sigma[i, 1] / (mysigma + self.sigma[i, 1])
            )
            row[counter + 2] = myindex
            col[counter + 2] = myindex + self.n
            data[counter + 2] = (
                mysigma * self.sigma[i + 1, 0] / (mysigma + self.sigma[i + 1, 0])
            )
            ...
            for j in range(1, self.n - 1):
                ...
                col[counter + 1] = myindex + 1
                data[counter + 1] = (
                    mysigma * self.sigma[i, j + 1] / (mysigma + self.sigma[i, j + 1])
                )
                row[counter + 2] = myindex
                col[counter + 2] = myindex + self.n
                data[counter + 2] = (
                    mysigma * self.sigma[i + 1, j] / (mysigma + self.sigma[i + 1, j])
                )
                row[counter + 3] = myindex
                col[counter + 3] = myindex - self.n
                data[counter + 3] = (
        ...
        self.V = scipy.sparse.linalg.spsolve(coefmatrix, -self.EMsource).reshape(
            (self.n, self.n)
        )
    ...
    def record(self, t):
        ...
        while not (recindex is None):
            if recinfo[0] == "SARvol":
                record = self.SAR[
                    recinfo[1][0] : recinfo[1][2], recinfo[1][1] : recinfo[1][3]
                ]
...
class EMsolverThread(threading.Thread):
    def __init__(
        ...
        self.myEMsolver = EMsolver(
            n, EMinit, dt, EMsource, Tinit, tend, sigma0, sigmaT, transmitter
        )
        self.name = "EMsolver"
