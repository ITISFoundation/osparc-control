...
class Controller:
    def __init__(
        ...
        self.iteration_time = (
            iteration_time  # Time resolution for recording/setting variables
        )
        self.tweakparam_key = (
            tweakparam_key  # Name of the variable that can be modified (set)
        )
        self.initval = initval  # Initial value for the variable `tweakparam_key`
        self.regulationparam_key = (
            regulationparam_key  # Name of the variable that can be recorded
        )
        ...
        self.controlledvals = (
            []
        )  # Initialize list for output controlled values (values of `regulationparam_key`)
    ...
    def run(self):
        ...
        recindex = self.controlled.record(
            self.regulationparam_key,
            self.iteration_time,
            self.regulationparam_otherparams,
        )
        ...
        while not self.controlled.endsignal:
            ...
            recindex = self.controlled.record(
                self.regulationparam_key, waittime, self.regulationparam_otherparams
            )
        ...
        return {
            "set value": self.sets,
            "errors": self.errors,
            "controlled value": self.controlledvals,
        }
...
if __name__ == "__main__":
    ...
    with control_interface:
        ...
        controller = Controller(
            "sourcescale", 1, "Tpoint", [10, 10], 4, 10, 0.01, 0.00, 0, transmitter
        )
        out = controller.run()
